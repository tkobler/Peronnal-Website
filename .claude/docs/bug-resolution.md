...
Your Name reports a bug. Claude does **not** immediately open a branch and start patching. Instead:
...
UI bug reports without a screenshot are incomplete. If Your Name can't provide one, Claude captures one. If neither can reproduce the bug visually, downgrade it to "needs reproduction" and stop.
...
1. The reproduction steps Your Name provided
...
When hypotheses actually contradict, say so and ask Your Name to decide which path to investigate first.

### 1.5 Stop conditions

Do NOT proceed to Phase 2 if:
- **Reproduction isn't reliable** — go back to Your Name for more details; don't fix what you can't trigger
...
If a test genuinely cannot be written (rare — usually a visual-only bug that doesn't warrant a full visual baseline), document **explicitly** in TRACKING.md why, and get Your Name's OK to proceed without one.
...
- **Symptom** — what Your Name (or the reporter) saw
